...
if ($line =~ m#((?:(?:[A-Z][A-Za-z]*::)+)\w+)\(#) {
	...
	my $cmd = "fdfind -c never -p '$file' $ENV{NIKE_HOME}/ | xargs grep -n -m 1 -H '^sub $function'";
	my @res = `$cmd`;
	...
	my $cmd = "fdfind -c never -p '$file' $ENV{NIKE_HOME}/";
	my @res = `$cmd`;
	...
	if (scalar(@res)) {
		...
		my $file = "lib/".join("/", @tokens).".pm";
		my $cmd = "fdfind -c never -p '$file' $ENV{NIKE_HOME}/ | xargs grep -n -m 1 -H '$const'";
		my @res = `$cmd`;
